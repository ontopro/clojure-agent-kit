...
| `bb report` | Render a run record (EDN on stdin): per-step time, cost, model and serving provider |
| `bb example` | End-to-end tour of the loop, no model calls |
| `bb run-loop` | Drive one task through the loop against real models — a command per step, triage by hand. See `dev/run_loop.clj` |
