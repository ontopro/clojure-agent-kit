...
A trivial Clojure project used as a fixture for the kit — see README.md. If you are reading
this headlessly, you were dispatched here with a task packet.

> The block below is generated from `harness-seed/resources/agent-rules.edn` by
> `bb rules-sync`. Everything outside the markers is hand-written and survives a sync.
...
| `bb nrepl` | nREPL on an OS-assigned port, written to `.nrepl-port` |
| `bb fmt` | Format in place |
